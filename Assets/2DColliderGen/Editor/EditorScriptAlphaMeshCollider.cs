...
public class EditorScriptAlphaMeshCollider : Editor {
	...
	public override void OnInspectorGUI() {
			...
			if (usedTexture == null) {
				...
				if (usedTextureIsCustomTex) {
					...
					if (otTilesSpriteSelectionStatus == OTTilesSpriteSelectionStatus.ALL) {
						EditorGUILayout.ObjectField("OTTilesSprite", usedTexture, typeof(Texture2D), false);
					}
					else if (isAtlas) {
		...
		if (canRecalculateAnyCollider) {
			...
			if (!isFirstEnabledMaxPointCountDifferent) {
				int newFirstEnabledMaxPointCount = EditorGUILayout.IntSlider("Outline Vertex Count", commonFirstEnabledMaxPointCount, 3, mPointCountSliderMax);
	...
	static void RemoveColliderAndGenerator(GameObject[] gameObjects) {
		foreach (GameObject gameObj in gameObjects) {
			...
			Transform meshColliderNode = null;
			if (alphaMeshColliderComponent) {
				meshColliderNode = alphaMeshColliderComponent.TargetNodeToAttachMeshCollider;
			...
			// Potentially a MeshColliderNode at a child node (used for SmoothMoves scale animation)
			if (meshColliderNode != null) {
				meshColliderComponent = meshColliderNode.GetComponent<MeshCollider>();
				if (meshColliderComponent) {
					DestroyImmediate(meshColliderComponent);
				}
			}
	...
	//-------------------------------------------------------------------------
	void PrepareColliderIslandsAtSelectedColliders(SortedList<int, AlphaMeshCollider> sortedTargets) {
		int lastHash = int.MinValue;
		foreach (KeyValuePair<int, AlphaMeshCollider> pair in sortedTargets) {
			int hash = pair.Key;
			AlphaMeshCollider target = pair.Value;
	...
	//-------------------------------------------------------------------------
	void PrepareColliderRegionsIfOldVersion(SortedList<int, AlphaMeshCollider> sortedTargets) {
	...
	//-------------------------------------------------------------------------
	void RecalculateOTTilesSpriteColliders(SortedList<int, AlphaMeshCollider> sortedTargets) {
		foreach (KeyValuePair<int, AlphaMeshCollider> pair in sortedTargets) {
			AlphaMeshCollider target = pair.Value;
